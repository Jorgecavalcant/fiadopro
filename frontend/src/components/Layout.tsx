    ...
    <div style={{ display: 'flex', height: '100%', overflow: 'hidden', background: '#F1F5F9' }}>
      ...
      <aside
        style={{
          ...
          transition: 'min-width 280ms cubic-bezier(.4,0,.2,1), width 280ms cubic-bezier(.4,0,.2,1)',
          zIndex: 50,
        }}
        className={`
      ...
      >
        ...
        <div style={{
          ...
          {!collapsed && (
            <div style={{ flex: 1, overflow: 'hidden' }}>
              <p style={{ color: 'white', fontWeight: 900, fontSize: 15, letterSpacing: '-0.02em', lineHeight: 1.15 }}>
                Fiado<span style={{ color: '#818CF8' }}>Pro</span>
              </p>
              <p style={{ color: 'rgba(148,163,184,0.65)', fontSize: 11, fontWeight: 600, lineHeight: 1 }}>
          ...
          <button
            ...
            style={{
              ...
              color: 'rgba(148,163,184,0.5)', transition: 'background 150ms, color 150ms',
              flexShrink: 0,
            }}
        ...
        {/* Nav */}
        <nav style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', padding: '16px 10px', display: 'flex', flexDirection: 'column', gap: 2 }}>
          {!collapsed && <NavLabel>Principal</NavLabel>}
          {collapsed && <div style={{ height: 8 }} />}
          {navMain.map(item => (
            <SidebarItem key={item.id} item={item} active={activeView === item.id} collapsed={collapsed} onClick={() => setActiveView(item.id)} />
          ))}
        ...
        {/* Footer */}
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.07)', padding: 10, display: 'flex', flexDirection: 'column', gap: 4 }}>
          {!isPro && !collapsed && (
            <button
              onClick={onUpgrade}
              style={{
                width: '100%', padding: '9px 14px', borderRadius: 12, border: 'none', cursor: 'pointer',
                background: 'linear-gradient(135deg,#6366F1,#8B5CF6)',
                color: 'white', fontWeight: 700, fontSize: 12,
                display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
                boxShadow: '0 4px 12px rgba(99,102,241,.35)', marginBottom: 4,
          ...
          <button
            ...
            style={{
              width: '100%', padding: collapsed ? '9px' : '8px 10px', borderRadius: 12, border: 'none', cursor: 'pointer',
          ...
          >
            <div className={`bg-gradient-to-br ${avatarColor}`} style={{
            ...
            {!collapsed && (
              <>
                <div style={{ flex: 1, overflow: 'hidden', textAlign: 'left' }}>
                  <p style={{ color: 'white', fontSize: 13, fontWeight: 700, lineHeight: 1.2 }} className="truncate">{user?.name ?? 'Usuário'}</p>
                  <p style={{ color: 'rgba(148,163,184,0.65)', fontSize: 11 }} className="truncate">{user?.email ?? ''}</p>
                </div>
                <Settings style={{ width: 15, height: 15, color: 'rgba(148,163,184,0.45)', flexShrink: 0 }} />
              </>
            )}
          </button>
          ...
          <button
            ...
            style={{
              ...
              background: 'transparent', display: 'flex', alignItems: 'center',
              gap: collapsed ? 0 : 10, justifyContent: collapsed ? 'center' : 'flex-start',
              color: 'rgba(252,165,165,0.8)', transition: 'background 150ms, color 150ms',
            }}
            onMouseEnter={e => { (e.currentTarget).style.background = 'rgba(239,68,68,0.1)'; (e.currentTarget).style.color = '#FCA5A5'; }}
            onMouseLeave={e => { (e.currentTarget).style.background = 'transparent'; (e.currentTarget).style.color = 'rgba(252,165,165,0.8)'; }}
          >
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, overflow: 'hidden' }}>
        ...
        <header style={{
          ...
          background: 'white', flexShrink: 0, zIndex: 30,
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <h1 style={{ fontSize: 17, fontWeight: 900, color: '#0F172A', letterSpacing: '-0.025em', margin: 0 }}>
              {pageTitle}
            </h1>
            {isPro && (
              <span style={{
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {pendingCount > 0 && (
              <button
                ...
                style={{
                  position: 'relative', width: 38, height: 38, borderRadius: 12, border: 'none', cursor: 'pointer',
                  background: 'transparent', color: '#64748B',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  transition: 'background 150ms',
                }}
            ...
            <button
              ...
              style={{
                ...
                padding: '4px 12px 4px 6px', borderRadius: 12, border: 'none', cursor: 'pointer',
            ...
            >
              ...
              <span className="hidden sm:block" style={{ fontSize: 14, fontWeight: 700, color: '#334155' }}>
        ...
        <main key={activeView} style={{ flex: 1, overflowY: 'auto' }}>
          <div style={{ maxWidth: 1280, margin: '0 auto', padding: '24px 20px' }} className="fp-view-enter">
            {children}
          </div>
        </main>
      </div>
...
interface SidebarItemProps {
  item: { id: AppView; label: string; icon: React.FC<any>; badge: number };
  active: boolean;
  collapsed: boolean;
  onClick: () => void;
}
  ...
  return (
    <button
      ...
      style={{
        ...
        border: 'none',
        cursor: 'pointer',
        background: active ? 'rgba(99,102,241,0.22)' : hover ? 'rgba(255,255,255,0.06)' : 'transparent',
    ...
    >
      {active && (
        <span style={{
          ...
          height: '60%', width: 3,
          background: '#818CF8', borderRadius: '0 4px 4px 0',
        }} />
      )}
