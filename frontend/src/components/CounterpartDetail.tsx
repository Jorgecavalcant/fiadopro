            ...
            <div key={t.id} className="bg-white rounded-xl shadow-sm border border-slate-100 p-4">
              <div className="flex justify-between items-start gap-3">
                ...
                <div className="text-right">
                  ...
                  <span
                    className={`inline-block mt-1 text-[10px] font-black uppercase px-2 py-0.5 rounded-full ${status.className}`}
                  >
                    {status.label}
                  </span>
                </div>
...
function PaymentModal({ openDebts, onClose, onSubmit }: PaymentModalProps) {
  ...
  const [paymentMethod, setPaymentMethod] = useState<
    'PIX' | 'CREDIT_CARD' | 'DEBIT_CARD' | 'COMPENSATION'
  >('PIX');
                ...
                >
                  ...
                  {openDebts.map((d) => (
                    <option key={d.id} value={d.id}>
                      {d.description || 'Dívida'} —{' '}
                      {new Date(d.occurred_at).toLocaleDateString('pt-BR')}
                    </option>
                  ))}
                </select>
              </div>
